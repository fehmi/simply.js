simply = {
	...
	setupInlineComps: function () {
		// for root (index.html etc)
		let simplyTemplates = document.querySelectorAll("template[simply]");
		if (simplyTemplates) {
			let doc;
			
			simplyTemplates.forEach(element => {
				let comp = element.parentElement;
				let regexstr = "get\(.*"+comp.tagName+".*\)";
				let regex = new RegExp(regexstr, "i");
				let match = document.querySelector("body").outerHTML.match(regex);
				...
				if (!match) {
					...
					simply.registerComponent({
						name: comp.tagName.toLowerCase(), //module.default.name,
						template: parsed.template,
						style: parsed.style,
						script: parsed.script,
						docStr: doc,
